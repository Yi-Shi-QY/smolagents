...
<Tip>
Note: `ToolCollection.from_hub()` has been removed. `ToolCollection` is primarily used for grouping locally defined or loaded tools, or tools from an MCP server.
</Tip>
